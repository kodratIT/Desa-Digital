        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('manage.jenis-surat.create') }}" class="btn btn-success mb-2 ">
                        Tambah
                    </a>
                </div>
                <div class="card-body ">
                    ...
                    <div class="table-responsive text-center">
                        <table class="table table-bordered border text-nowrap mb-0 " id="new-edit">
                            ...
                            @if ($let == null)
                                <tr>
                                    <td colspan="6" class="text-gray">DATA TIDAK DITEMUKAN</td>
                                </tr>
                            @else                                         
                                @foreach ($latters as $latter) 
                                    <tr>
                                        ...
                                        <td>{{ $latter->name }}</td>
                                        <td>{{ $latter->username }}</td>
                                        <td>{{ $latter->created_at }}</td>
                                        <td>{{ $latter->updated_at }}</td>
                                        <td class="d-flex justify-content-center border-0">
                                            <a href="{{route('manage.jenis-surat.edit',Crypt::encrypt($latter->id))}}" class="btn btn-sm btn-primary badge  mx-2"><i class="fe fe-edit"></i></a>
                                            <button class="btn-sm btn-danger badge" data-bs-toggle="modal" data-bs-target="#smallmodal-{{ $latter->id }}"><i class="fa fa-trash"></i></button>

                                            <div class="modal  fade" id="smallmodal-{{ $latter->id }}" tabindex="-1" role="dialog">
                                                <div class="modal-dialog modal-sm" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            <form action="{{route('manage.jenis-surat.destroy',Crypt::encrypt($latter->id))}}" method="post" class="inline-block">
                                                                @csrf
                                                                @method('DELETE')
