        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title ">List Nama Desa</h3>
                    <span class="mx-auto"></span>
                    <a href="{{ route('manage.data-desa.create') }}" class="btn btn-success mb-2 ">
                        Tambah
                    </a>
                </div>
                <div class="card-body ">
                    ...
                    <div class="table-responsive text-center">
                        <table class="table table-bordered border text-nowrap mb-0 " id="new-edit">
                            ...
                            @else                                         
                                @foreach ($vilages as $vilage) 
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $vilage->name_desa }}</td>
                                        <td>{{ $vilage->created_at }}</td>
                                        <td>{{ $vilage->updated_at }}</td>
                                        <td class="d-flex justify-content-center border-0">
                                            <a href="{{route('manage.data-desa.edit',Crypt::encrypt($vilage->id))}}" class="btn btn-sm btn-primary badge  mx-2"><i class="fe fe-edit"></i></a>

                                            <button class="btn-sm btn-danger badge" data-bs-toggle="modal" data-bs-target="#smallmodal-{{ $vilage->id }}"><i class="fa fa-trash"></i></button>

                                            <div class="modal  fade" id="smallmodal-{{ $vilage->id }}" tabindex="-1" role="dialog">
                                                <div class="modal-dialog modal-sm" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            <form action="{{route('manage.data-desa.destroy',Crypt::encrypt($vilage->id))}}" method="post" class="inline-block">
                                                                @csrf
                                                                @method('DELETE')
